        ...
        if login_serializer.is_valid():
            ...
            if user.is_active:
                ...
                return Response({'error':'Este usuario no puede iniciar sesion'},
                                    status= status.HTTP_401UNAUTHORIZED)

        else:
            return Response({'error':'Nombre de usuario o contraseña incorretos.'},
...
class Logout(APIView):
    ...
    def post(self, request, *args,**kwargs):
        try:
            ...
            return Response({'error':'Nos se ha encontrado token en la peticion.'},status=status.HTTP_409_CONFLICT)
